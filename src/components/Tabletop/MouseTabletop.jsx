...
export function useMouseTabletop({
    ...
    const isTokenBloqueado = useCallback((tokenId) => uiState.tokensBloqueados[tokenId] === true, [uiState.tokensBloqueados]);
    ...
    const handleMouseDown = useCallback((event) => {
        ...
        if (event.button === 2) {
            ...
            if (tokenSobre) {
                ...
                uiDispatch({
                    ...
                    payload: {
                        token: tokenSobre, mouseX, mouseY,
                        timestamp: Date.now(),
        ...
        if (event.button === 0 && !fov.modoDesenho) {
            ...
            if (tokenSobre) {
                if (isTokenBloqueado(tokenSobre.token.id)) {
                ...
                if (canto) {
                    iniciarRedimensionamento(tokenSobre.token, tokenSobre.indice, canto, { x: mouseX - posicao.x, y: mouseY - posicao.y });
                    event.preventDefault();
                    return;
                }
                ...
                if (emitirSelecao && !dragInProgressRef.current && !resizeInProgressRef.current) {
                    emitirSelecao(tokenSobre.token.id);
                }
        ...
        iniciarArrastoToken, emitirSelecao, dragInProgressRef, resizeInProgressRef]);
    ...
    const handleMouseUp = useCallback((event) => {
        ...
        if (event.button === 2) {
            if (uiState.mouseDownInfo?.isBlocked && !isRightClickDragRef.current) {
                const tokenSobre = uiState.mouseDownInfo.token;
                uiDispatch({ type: 'SELECT_TOKEN', payload: uiState.mouseDownInfo.tokenIndice });
                uiDispatch({
                    type: 'OPEN_CONTEXT_MENU',
                    payload: { aberto: true, x: event.clientX, y: event.clientY, tokenIndice: uiState.mouseDownInfo.tokenIndice, tokenId: tokenSobre.token.id, token: tokenSobre.token }
                });
            }
            ...
            if (uiState.mouseDownInfo && uiState.ui.isClickingToken && uiState.mouseDownInfo.isRightClick && !uiState.mouseDownInfo.isBlocked) {
                ...
                uiDispatch({
                    type: 'OPEN_CONTEXT_MENU',
                    payload: { aberto: true, x: event.clientX, y: event.clientY, tokenIndice: tokenSobre.indice, tokenId: tokenSobre.token.id, token: tokenSobre.token }
                });
            }
            isRightClickDragRef.current = false;
        }
